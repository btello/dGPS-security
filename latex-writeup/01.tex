...
In our attack, we saw that we can send arbitrary correction data from a mountpoint that will travel through an Ntrip network to affect a client requesting GPS correction data.  A client is directly affected by RTCM3 correction data that we send from the NetRS device and can be tricked into recalculating its own position.  A stronger and simpler attack would be to masquarade via a Man in the Middle attack as an Ntrip Caster due to the lack of mutual authentication.  We do not go into the specific details of a MitM attack since much research is done in this area and the attack is trivial.
...
Investigate whether or not any real life attacks against differential GPS have ever occurred and what the objectives were.

\section{Conclusion}

We have discovered a Diffierential GPS system that is widely used yet highly insecure.  It is important for all users and implementors of differential GPS technologies to make security more of a priority as cyberphysical security is an area of active research and interest.
